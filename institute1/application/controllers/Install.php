...
class Install extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->router->default_controller == 'install' or $this->router->default_controller == 'install/index') {
...
//        session_start();
    ...
    function finalizing_setup($param1 = '', $param2 = '')
    {
        ...
        if ($param1 == 'setup_admin') {
            $admin_data['firstname'] = html_escape($this->input->post('first_name'));
            $admin_data['lastname']  = html_escape($this->input->post('last_name'));
...
//            $admin_data['social_links'] = json_encode($social_links);
            ...
            $data['institute_name'] = $this->input->post('system_name');
...
//        session_start();
    ...
    function configure_routes()
    {
        ...
        $data_routes = str_replace('install', 'frontend', $data_routes);
    ...
    {
        // write autoload.php
        $data_routes = file_get_contents('./application/config/autoload.php');
        $data_routes = str_replace('paypal', 'database', $data_routes);
        file_put_contents('./application/config/autoload.php', $data_routes);
    }
